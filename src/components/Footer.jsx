            ...
            <div className="flex flex-wrap items-center gap-4 mb-6">
                ...
                <div className="w-48 bg-white rounded-xl overflow-hidden border border-white/10 shadow-xl">
                    ...
                    <div className="p-3 relative -mt-6">
                      <div className="w-10 h-10 rounded-full bg-white border-2 border-white flex items-center justify-center mb-2 relative z-10 shadow-sm">
                        <FaLinkedin className="w-6 h-6 text-[#042244]" />
                      </div>
              ...
              >
                 <div className="w-52 bg-[#0b141a] border border-[#25D366]/20 rounded-xl shadow-2xl overflow-hidden">
                    ...
                    <div className="p-4 bg-[#0b141a] bg-opacity-50">
              ...
              <HoverableSocialButton icon={Mail} label="Email"
...
const HoverableSocialButton = ({ icon: Icon, href, label, brandColor, align = "center", children }) => {
  const [isHovered, setIsHovered] = useState(false);
  const isMail = href.startsWith('mailto:');

  const handleInteraction = (e) => {
    // If it's a touch device and it's not hovered, prevent default and show tooltip
    if (window.innerWidth < 768 && !isHovered) {
      e.preventDefault();
      setIsHovered(true);
      // Automatically hide after 3 seconds on mobile
      ...
      <AnimatePresence>
        {isHovered && (
          <motion.div
            ...
            className={`absolute bottom-14 z-50 whitespace-nowrap ${
              align === 'left' ? '-left-4 md:left-0 origin-bottom-left' : 
              align === 'right' ? '-right-6 md:right-0 origin-bottom-right' : 
